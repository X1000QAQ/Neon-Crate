...
#   以保证 API 在刮削高压期仍能毫秒级返回，不被 DB/CPU 抢占。
_MANAGED_PATHS_TTL_SECONDS = 30.0
_managed_paths_cache: list[Path] | None = None
_managed_paths_cache_at: float = 0.0
_managed_paths_refresh_lock = asyncio.Lock()


def _path_is_under_any_base(target: Path, bases: list[Path]) -> bool:
    """
    判断 target 是否位于任一 base 目录下（带路径边界，避免 /base 与 /base2 前缀碰撞）。
    """
    t = target.as_posix().rstrip("/")
    for b in bases:
        base = b.as_posix().rstrip("/")
        if t == base or t.startswith(base + "/"):
            return True
    return False


async def _get_managed_paths_cached(logger: logging.Logger) -> list[Path]:
    """
    获取 managed paths 白名单（带 TTL 缓存 + 单飞刷新）。

    注意：DB 读取是同步调用，必须放到线程中执行，且刷新频率必须被 TTL 限制，
    否则高并发图片请求会放大线程池压力。
    """
    global _managed_paths_cache, _managed_paths_cache_at

    now = time.monotonic()
    if _managed_paths_cache and (now - _managed_paths_cache_at) < _MANAGED_PATHS_TTL_SECONDS:
        return _managed_paths_cache

    # singleflight：只允许一个协程刷新缓存，其余协程直接复用旧缓存（若存在）
    if _managed_paths_refresh_lock.locked() and _managed_paths_cache:
        return _managed_paths_cache

    async with _managed_paths_refresh_lock:
        now = time.monotonic()
        if _managed_paths_cache and (now - _managed_paths_cache_at) < _MANAGED_PATHS_TTL_SECONDS:
            return _managed_paths_cache
        ...
        def _load_paths_sync() -> list[Path]:
        ...
        try:
            allowed_paths = await anyio.to_thread.run_sync(_load_paths_sync)
        except Exception as e:
            logger.warning(f"[SECURITY] 白名单刷新失败，继续使用旧缓存（若有）: {e}")
            return _managed_paths_cache or []
        ...
        _managed_paths_cache_at = time.monotonic()
        return all_allowed
...
async def get_stats(db: DbDep):
    """
    获取控制台统计数据（缓存模式，避免对 NAS 产生实时 I/O 压力）。

    业务链路：
    从配置缓存读取 movies/tv_shows 计数 → 从 tasks 表读取流水线实时统计（无磁盘 I/O）→ 返回 StatsResponse。

    Args:
        db: 数据库依赖注入（DbDep），用于读取配置缓存与 dashboard 统计。

    Returns:
        StatsResponse: 大屏统计数据（movies/tv_shows/pending/completed）。
...
async def get_logs(
    ...
    """
    读取系统日志（最后 1000 行，支持 tag 过滤）。

    业务链路：
    从日志文件读取最近 N 行 → 解析为结构化条目 → 按 tags 过滤 → 生成稳定的条目 id（避免前端 key 冲突）→ 返回。

    Args:
        tags: 可选。逗号分隔的标签过滤（如 SCAN,TMDB,SUBTITLE,ERROR,API...）。

    Returns:
        Dict[str, Any]:
            - logs: 结构化日志列表（含 id/timestamp/level/message/tag）
            - source: 日志文件路径
            - total: 返回条目数

    Raises:
        HTTPException:
            - 500: 日志文件读取失败或解析过程中出现运行时异常。
    """
