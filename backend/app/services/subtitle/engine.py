"""
字幕引擎 - OpenSubtitles 检索、评分、下载与状态回写。

职责：
- 根据任务中的 IMDb / TMDB ID、媒体类型和季集坐标搜索字幕。
- 优先选择简体中文字幕，过滤听障特效字幕，并按语言优先级打分。
- 下载最佳候选字幕，按 `视频名.ai.语言码.扩展名` 规范落盘。
- 将字幕状态写回热表或冷表，供前端展示和后续任务跳过。

外部 API 边界：
- OpenSubtitles 免费账号存在严格频率限制，调用方会串行任务并追加休眠。
- 本模块内部对 429 和 5xx 做指数退避；401、403、配额耗尽等全局错误会触发熔断。
- 无有效 IMDb / TMDB ID 时必须带 query 参数，避免 OpenSubtitles 返回 400。

落盘边界：
- 已归档任务优先使用 `target_path`，确保字幕写入媒体库目录而不是下载目录。
- 已存在同名前缀字幕时直接短路，避免重复下载。
"""
import asyncio
import os
import re
...
class SubtitleEngine:
    ...
    本类负责单条任务的字幕检索和落盘：
    - 构建 OpenSubtitles 查询参数。
    - 过滤听障字幕。
    - 按语言和格式评分选择最佳候选。
    - 下载字幕并写回任务状态。

    全量字幕补完任务的并发控制和任务间休眠由路由层负责。
    """
