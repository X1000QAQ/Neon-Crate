...
flowchart TD
    ...
    E -->|"传递: 6字段 JSON\n{query, type, season, episode,\nfilename_year, knowledge_year}\n若 DB media_type=mixed，则 type 用于最终裁决"| F
...
| DB `media_type` | AI `type` 处理 |
|---|---|
| `movie` / `tv` | 路径权威锁死，忽略 AI 的相反建议 |
| `mixed` | 采用 AI 返回的合法 `movie` / `tv` 作为最终 `refined_type` |
| 其他/空值 | AI 合法则采用，否则 fail-safe 降级为 `movie` |
