/**
 * LoginPage - 登录与首次初始化页面
 *
 * 负责判断系统是否已初始化，并根据状态展示初始化表单或登录表单。
 * 页面包含启动动画、输入框聚焦效果、密码显示切换和错误提示。
 *
 * 新手提示：
 * - `api.authStatus()` 决定当前是初始化模式还是登录模式。
 * - 初始化成功或登录成功后，后端会返回 Token，前端保存后进入主控制台。
 * - 启动动画只负责视觉展示，不影响真实认证逻辑。
 */
'use client';
