...
export default function RegexLab({ t, config, setConfig }: Props) {
  ...
  const setRegexRules = (value: string) => {
    if (!config) return;
    setConfig({
      ...config,
      settings: {
        ...config.settings,
        filename_clean_regex: value,
      },
    });
  };
    ...
    try {
      ...
      while ((m = re.exec(testInput)) !== null) {
        ...
        if (m.index > lastIndex) {
          parts.push({ text: testInput.slice(lastIndex, m.index), matched: false });
        }
      ...
      if (lastIndex < testInput.length) {
        parts.push({ text: testInput.slice(lastIndex), matched: false });
      }
  ...
  const handleReset = async () => {
    if (!confirm(t('regex_reset_confirm'))) {
      return;
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <NeuralSection title={t('regex_system_rules')}>
        <NeuralTextarea
          label={t('regex_filename_clean_regex')}
          value={regexRules}
          onChange={(e) => setRegexRules(e.target.value)}
          rows={12}
          className="text-sm resize-y"
          placeholder="# 每行一条正则规则"
        />
      ...
      <button
        onClick={testRegex}
        className="w-full py-3 bg-transparent border-2 border-cyber-cyan text-cyber-cyan font-bold uppercase tracking-widest hover:bg-cyber-cyan hover:text-black transition-all"
        style={{
              ...
              <div className="font-mono text-sm whitespace-pre-wrap break-all">
                {highlightParts.map((p, idx) => (
                  <span
                    ...
                    className={
                      p.matched
                        ? 'bg-[rgba(0,230,246,0.25)] text-cyber-cyan px-0.5 line-through decoration-cyber-cyan'
                        : ''
                    }
                  >
                    {p.text}
      ...
      <NeuralSection title={t('regex_common_title')}>
        <div className="space-y-2 text-sm text-cyber-cyan/70">
          <div>
            <code className="text-cyber-cyan">\\.(\\d{4})\\.</code> - {t('regex_pattern_year')}
          </div>
          <div>
            <code className="text-cyber-cyan">\\.(1080p|720p|4K)\\.</code> - {t('regex_pattern_res')}
          </div>
          <div>
            <code className="text-cyber-cyan">\\.(BluRay|WEB-DL|HDTV)\\.</code> - {t('regex_pattern_source')}
          </div>
        </div>
      </NeuralSection>
    </div>
