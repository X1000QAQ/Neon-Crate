/**
 * DownloadConfirmOverlay - 下载授权全屏确认弹窗
 *
 * 当 AI 助手识别到“下载影片/剧集”的意图时，本组件负责展示最终确认界面。
 * 用户可以在这里查看资源海报、标题、年份、简介以及重复资源警告，再决定是否授权下载。
 *
 * 新手提示：
 * - `pending` 是后端返回的待确认操作载荷，包含 TMDB 元数据。
 * - `confirmLoading=true` 时应禁用取消行为，避免确认请求和取消请求并发打架。
 * - 此组件只负责展示和回调，不直接调用下载接口。
 */
'use client';
