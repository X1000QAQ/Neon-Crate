...
interface MediaTableProps {
  ...
  onUnignore: (filePath: string) => Promise<void> | void;
  onRebuild: (params: {
    task_id: number;
    is_archive: boolean;
    media_type: string;
          ...
          return (
            <div key={group.key} className="flex items-stretch gap-2">
              ...
              <div className="flex-1 min-w-0">
                <MediaRow level={0} title={resolveDisplayTitle(rep, configPaths)} subtitle={subtitleOf(rep)} status={rep.status} task={rep} onDelete={() => deleteIds([rep.id])} onIgnore={rep.file_path ? () => onIgnore(rep.file_path!) : undefined} onUnignore={rep.file_path ? () => onUnignore(rep.file_path!) : undefined} onRetry={onRetry} onRebuildClick={openRebuild} processingId={processingId} setProcessingId={setProcessingId} />
              </div>
            </div>
          );
        }
